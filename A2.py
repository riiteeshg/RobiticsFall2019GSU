...
MIN_DISTANCE = 125 # measured in mm
NO_OBSTACLE = 3000
...
def Main():
    ...
    try:
        ...
        distanceReading = gopigo3.init_distance_sensor()
        gopigo3.forward()
    ...
    while robot_operating:
        ...
        #distanceReading = distance_sensor.read_nm()
        determined_speed = 0
        ...
        # if the robot is closer to the target
            ...
            file.write('"Row", "encoder", "distance\n"')

            print('"Row", "encoder", "distance"')
            #print( "control")
            while turncount < 4:
                gopigo3.drive_cm(50, blocking = False) #will not wait for 
                previousReading = -500
                er= gopigo3.read_encoders_average()
                print("Encoder Reading", gopigo3.read_encoders_average())
                while not previousReading == encoderReading: 
                        print ("Running")
                        #distanceReading = my_distance_sensor.read_nm()
                        previousReading = encoderReading
                        encoderReading = gopigo3.read_encoders_average()
                        rownum += 1
                        file.write("{:4}, {:4} ,{:4} \n".format(rownum, er, current_distance))
                        print(("{:3}, {:3}, {:3}" .format(rownum, er, current_distance )))
                        ...
                        #outputFile.write("{}, {} ,{}\n".format(rownum, encoderReading, current_distance))
            outputFile.close() 
            gopigo3_stationary = True
            #gopigo3.stop()
